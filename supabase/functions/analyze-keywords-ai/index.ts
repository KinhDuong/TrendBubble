...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Breakout/Hidden Gems: Keywords with moderate-low current volume (<50K searches/month),
    // low-medium competition, and early growth signals (20–50%+ YoY)
    const breakoutKeywords = keywords.filter(kw => {
      const hasModerateVolume = kw.searchVolume < 50000;
      const hasLowMediumCompetition = kw.competition &&
        (kw.competition === 'Low' || kw.competition === 'Medium' ||
         kw.competition === 'low' || kw.competition === 'medium' ||
         (typeof kw.competition === 'number' && kw.competition < 0.6));
      const hasGrowthSignal = (kw.yoyChange && kw.yoyChange >= 0.2) ||
                              (kw.threeMonthChange && kw.threeMonthChange >= 0.15);

      // Look for informational/commercial intent keywords
      const keyword = kw.keyword.toLowerCase();
      const hasInfoIntent = keyword.includes('benefits') || keyword.includes('best') ||
                           keyword.includes('how to') || keyword.includes('what is') ||
                           keyword.includes('guide') || keyword.includes('tips') ||
                           keyword.includes('review') || keyword.includes('vs') ||
                           keyword.includes('compare') || keyword.includes('top');

      return hasModerateVolume && hasLowMediumCompetition && hasGrowthSignal && hasInfoIntent;
    })
    .sort((a, b) => (b.yoyChange || b.threeMonthChange || 0) - (a.yoyChange || a.threeMonthChange || 0))
    .slice(0, 15);
    ...
    if (breakoutKeywords.length > 0) {
      strategicInsights += '\n**Breakout/Hidden Gems Potential:**\n';
      strategicInsights += `(Keywords with <50K volume, low-medium competition, 20%+ growth, and informational/commercial intent)\n`;
      breakoutKeywords.forEach(kw => {
        const growth = kw.yoyChange ? `+${(kw.yoyChange * 100).toFixed(0)}% YoY` : `+${(kw.threeMonthChange! * 100).toFixed(0)}% 3-mo`;
        strategicInsights += `- "${kw.keyword}" (~${kw.searchVolume.toLocaleString()} searches, ${growth}, ${kw.competition} competition`;
        if (kw.bidHigh && kw.bidHigh > 0) strategicInsights += `, $${kw.bidHigh.toFixed(2)} CPC`;
        strategicInsights += `)\n`;
      });
    }
