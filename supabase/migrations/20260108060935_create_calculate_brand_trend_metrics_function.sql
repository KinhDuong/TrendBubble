/*
  # Create Brand Trend Metrics Calculation Function
  
  ## Purpose
  Calculates historical trend metrics for brand keyword data to support intensity scoring.
  Analyzes up to 48 months of monthly search volume data per keyword.
  
  ## What It Does
  - Extracts monthly search volumes from "Searches: Month Year" columns
  - Calculates linear regression slope (monthly growth rate)
  - Calculates R² (coefficient of determination - trend strength)
  - Counts "Rising Stars" (keywords with slope > 7% monthly AND R² > 0.7)
  - Aggregates metrics at brand level
  
  ## Returns
  - brand: Brand name
  - avg_slope: Average monthly growth rate across all keywords (as decimal)
  - avg_r_squared: Average R² across all keywords (0-1)
  - rising_stars_count: Number of keywords with strong positive trends
  - keywords_analyzed: Total keywords with sufficient data
  - avg_three_month_change: Average 3-month change (for comparison)
  
  ## Usage
  SELECT * FROM calculate_brand_trend_metrics('nike', 'user-uuid');
*/
...
GRANT EXECUTE ON FUNCTION calculate_brand_trend_metrics(text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION calculate_brand_trend_metrics(text, uuid) TO anon;
