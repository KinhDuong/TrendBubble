/*
  # Add Demand Score to Brand Comparison Stats

  ## Overview
  Updates the calculate_brand_comparison_stats function to include average demand score
  for each brand in the comparison.

  ## Changes
  - Adds "avgDemandScore" field to calculate average demand_score for all keywords
  - Only includes non-null demand scores in the average

  ## Usage
  The avgDemandScore provides a brand-level aggregate of keyword demand scores,
  helping users understand overall demand quality across all keywords for each brand.
*/
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      -- Average demand score (only non-null values)
      COALESCE(
        AVG(
          CASE
