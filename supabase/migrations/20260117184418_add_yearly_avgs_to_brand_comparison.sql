/*
  # Add Yearly Averages to Brand Comparison

  ## Overview
  Updates the calculate_brand_comparison_stats function to return yearly average columns
  for use in long-term trend calculations on the frontend.

  ## Changes
  - Return yearly average columns (2020-2026) for each brand
  - These will be used for calculating year-over-year trend slopes
  - Provides more stable long-term trend analysis compared to monthly regression

  ## Usage
  Frontend can now calculate long-term trends based on yearly averages:
  - 2022 Avg → 2023 Avg → 2024 Avg shows multi-year growth pattern
  - More stable than monthly slope calculations
  - Better represents sustained growth trends
*/
  ...
  FROM (
    ...
    FROM (
      SELECT
        ...
        -- Yearly averages from brand keyword (exact match)
        COALESCE(
          (SELECT "2020 Avg"
           FROM brand_keyword_data
           WHERE brand = bkd.brand
