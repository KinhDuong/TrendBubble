/*
  # Fix Brand Comparison Stats Function - Type Casting

  1. Changes
    - Fix type mismatches by properly casting text columns to numeric before comparisons
    - Handle NULL values safely
    - Ensure all numeric operations use proper type casting

  2. Details
    - All column comparisons now cast to numeric before comparing with numeric values
    - Handles edge cases where text values cannot be converted to numbers
*/
    ...
    SELECT
        ...
        AVG(
          CASE
            WHEN bkd."Competition (indexed value)" IS NOT NULL
              AND bkd."Competition (indexed value)" ~ '^[0-9]+\.?[0-9]*$'
              AND CAST(bkd."Competition (indexed value)" AS numeric) != 0
        ...
        AVG(
          CASE
            WHEN bkd."Top of page bid (low range)" IS NOT NULL
              AND bkd."Top of page bid (low range)" ~ '^[0-9]+\.?[0-9]*$'
              AND CAST(bkd."Top of page bid (low range)" AS numeric) != 0
        ...
        AVG(
          CASE
            WHEN bkd."Top of page bid (high range)" IS NOT NULL
              AND bkd."Top of page bid (high range)" ~ '^[0-9]+\.?[0-9]*$'
              AND CAST(bkd."Top of page bid (high range)" AS numeric) != 0
        ...
        AVG(
          CASE
            WHEN bkd."Three month change" IS NOT NULL
              AND bkd."Three month change" ~ '^-?[0-9]+\.?[0-9]*$'
              AND CAST(bkd."Three month change" AS numeric) != 0
        ...
        AVG(
          CASE
            WHEN bkd."YoY change" IS NOT NULL
              AND bkd."YoY change" ~ '^-?[0-9]+\.?[0-9]*$'
              AND CAST(bkd."YoY change" AS numeric) != 0
      ...
      COUNT(*) FILTER (
        WHERE (
          ...
          AND bkd."Three month change" ~ '^-?[0-9]+\.?[0-9]*$'
          AND CAST(bkd."Three month change" AS numeric) > 0.2
        ) OR (
          bkd."YoY change" IS NOT NULL
          AND bkd."YoY change" ~ '^-?[0-9]+\.?[0-9]*$'
          AND CAST(bkd."YoY change" AS numeric) > 0.5
        )
      ) as "risingStars",
      ...
      COUNT(*) FILTER (
        WHERE bkd."Competition (indexed value)" IS NOT NULL
          AND bkd."Competition (indexed value)" ~ '^[0-9]+\.?[0-9]*$'
          AND CAST(bkd."Competition (indexed value)" AS numeric) > 0.7
      ) as "highIntent",
