/*
  # Fix R² Calculation in Brand Trend Metrics
  
  ## Changes
  - Fix R² (coefficient of determination) calculation
  - Use proper linear regression formulas
  - Correct residual sum of squares calculation
*/
...
DECLARE
  ...
  i integer;
BEGIN
  -- Define all possible monthly search columns (48 months)
  ...
  LOOP
    -- Extract monthly values for this keyword
    EXECUTE format('
      SELECT ARRAY[%s] 
    ...
    -- Calculate slope (raw change per month)
    slope := (sum_xy - n * mean_x * mean_y) / NULLIF(sum_xx - n * mean_x * mean_x, 0);
    
    -- Calculate intercept
    intercept := mean_y - slope * mean_x;

    -- Calculate R² using correlation coefficient method
    -- r² = (n*Σxy - Σx*Σy)² / [(n*Σx² - (Σx)²) * (n*Σy² - (Σy)²)]
    IF (sum_xx - n * mean_x * mean_x) > 0 AND (sum_yy - n * mean_y * mean_y) > 0 THEN
      r_squared := POWER(sum_xy - n * mean_x * mean_y, 2) / 
                   ((sum_xx - n * mean_x * mean_x) * (sum_yy - n * mean_y * mean_y));
    ...
    -- Convert slope to percentage growth per month (after R² calculation)
    IF mean_y > 0 THEN
